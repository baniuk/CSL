...
{
	if(nrhs!=1)
        mexErrMsgIdAndTxt("M_WeldDetectApprox_DLL_Wrapper","One input required.");
    if(nlhs!=1)
        mexErrMsgIdAndTxt("M_WeldDetectApprox_DLL_Wrapper","One output required.");
	if (mxGetClassID(prhs[0]) != mxDOUBLE_CLASS)
    	mexErrMsgIdAndTxt("M_WeldDetectApprox_DLL_Wrapper","Input must be DOUBLE class.");
	...
	// debug - copy in->out
	memcpy(output_image, input_image, cols*rows);
...
// 		mexErrMsgIdAndTxt("M_WeldDetectApprox_DLL_Wrapper", "Error in Start().");
//  
