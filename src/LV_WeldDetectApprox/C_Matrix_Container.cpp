...
// Ver 1.3.3 - zmieniono readbinary - zachowana kompatybilność z wersja poprzednią
...
C_Matrix_Container::~C_Matrix_Container(void)
{
	if(data) {delete[] (double*)data;     data=nullptr;}
}
...
{
	// exportuje dane do wskaznika matlaba - do użycia w mexach bo w matylabie macierze są w kolumnach a w Containerach w rzędach
#ifdef _DEBUG
	if(data==nullptr)
...
{
	// importuje dane z wskaznika matlaba - do użycia w mexach bo w matylabie macierze są w kolumnach a w Containerach w rzędach
	unsigned int r,c;
	unsigned long a=0;
...
BOOL C_Matrix_Container::Dump(char *filename)
{
	...
	{
		if(data==nullptr) {fprintf_s(stream,"Empty Matrix"); return FALSE;}
		fprintf_s(stream,"[");
...
double C_Matrix_Container::Mean(C_Matrix_Container* out,unsigned int col)
	// liczy średnią z kolumny col.
	// Jeśli *out = nullptr to liczy z kolumny col i zwraca
...
double C_Matrix_Container::Median(void)
{
	...
	if(data==nullptr)
		_RPTF0(_CRT_ASSERT, "Matrix not initialized!!\n");
#endif
	// zwraca medianę ze wszystkich danych - dane raczej jako pojedyńczy wektor
	int sr;
	C_Matrix_Container tmp;
	CloneObject(&tmp);
...
// uwaga - modyfikuje tablicę !! dla parzystych zwraca niższy index
